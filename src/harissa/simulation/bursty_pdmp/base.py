...
def _step_jit(
    ...
    # 2. Compute the next jump
    v = kon(state[1], basal, inter, k0, k1)
...
class BurstyPDMP(Simulation):
    ...
    def use_numba(self, use_numba: bool) -> None:
        global _numba_functions, _kon_jit, _kon_bound_jit, _step_jit
        ...
        if self._use_numba != use_numba:
            if use_numba and _numba_functions[True] is None:
                ...
                _numba_functions[True] = {
                    'kon': _kon_jit,
                    'kon_bound': _kon_bound_jit,
                    'flow': njit()(flow)
                }
                for name, jited_f in _numba_functions[True].items():
                    globals()[name] = jited_f

                _step_jit = njit()(_step_jit)
                _numba_functions[True]['step'] = _step_jit
                globals()['step'] = _step_jit
                _numba_functions[True]['simulation'] = njit()(simulation)
    ...
    def run(self,
        ...
        states, phantom_jump_count, true_jump_count = simulation(
            state=initial_state,
